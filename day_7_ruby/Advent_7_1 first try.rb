class DirNode
    attr_reader :path, :size, :parent, :total_size, :directory_nodes, :file_names, :directory_names
    def initialize(name, data, depth = 0, parent = nil, debug = false)
        @name = name
        @data = data
        @depth = depth
        @parent = parent
        ...
        @directory_names.each do |dir|
            @directory_nodes[dir] = DirNode.new(dir, data[line_count..-1], depth + 1, self, debug)
        end
